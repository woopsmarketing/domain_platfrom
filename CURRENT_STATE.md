...
| `/market-history` | 낙찰 이력 테이블, `SoldAuctionsClient`에 위임, 24h 잠금 UI 포함 |
| `/auctions` | 실시간 경매 전용 페이지, `AuctionPageClient`에 위임, 30초 폴링 |
| `/tools` | 벌크 분석 / 도메인 비교 / TLD 통계 탭 통합 + 2개 신규 도구 카드 |
...
| sold_auctions UNIQUE(domain, platform) 제약 | ❌ DB에 미적용 (P0) |
| Supabase 환경변수 (URL, ANON_KEY, SERVICE_ROLE_KEY) | ✅ 입력 완료 |
| RAPIDAPI_KEY (도메인 SEO 지수) | ✅ 구독 완료 |
| WHOIS_API_KEY (Whois 조회) | ⚠️ 값 확인 필요 (키 없으면 null 반환) |
...
| `sold_auctions` | 낙찰 확정 도메인 저장 (migration.sql DDL 포함 완료, UNIQUE 제약 미적용) |
...
- [ ] **sold_auctions UNIQUE(domain, platform) 제약** → Supabase SQL Editor에서 적용 필요 (P0)
- [ ] **미커밋 변경사항 커밋** — active-auctions upsert, sold-domains limit 제거 (P0)
- [ ] OPENAI_API_KEY 발급 → domain-generator가 항상 fallback으로만 동작 중
- [ ] WHOIS_API_KEY 값 확인 → .env.local 및 Vercel 환경변수 점검
- [ ] DATABASE_URL / REDIS_URL / REDIS_TOKEN 잔류 키 정리 (코드 미사용)
- [ ] Railway 배포 (Watcher 상시 실행)
- [ ] 블로그 추가 콘텐츠 (4~8편)
- [ ] godaddy_live.py 검증
- [ ] 24시간 이전 낙찰 이력 프리미엄 잠금 실제 접근 제어 (예고 UI만 완료)
